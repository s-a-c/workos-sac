...
/**
 * Chinook Artists Seeder
 *
 * Seeds the artists table with data from the original Chinook database.
 * Maintains original IDs for foreign key compatibility with albums and tracks.
 */
class ChinookArtistsSeeder extends Seeder
{
    use ChinookSeederHelpers;
    /**
     * The original Chinook artists data.
     */
    private array $chinookArtists = [
        1 => 'AC/DC',
        2 => 'Accept',
        3 => 'Aerosmith',
        4 => 'Alanis Morissette',
    ...
    public function run(): void
    {
        ...
        $this->command->info('🎤 Starting Chinook Artists seeding...');

        // Check if we should skip seeding
        if ($this->shouldSkipSeeding('artists', 'Chinook Artists')) {
            return;
        }
        ...
        $this->executeInTransaction(function () {
            ...
            $artistsData = [];
            foreach ($this->chinookArtists as $originalId => $artistName) {
                $artistsData[] = [
                    'id' => $originalId,
                    'name' => $artistName,
                    'biography' => null,
            ...
            return $results;
        }, 'Chinook Artists seeding');

        $duration = microtime(true) - $startTime;
        $this->logSeedingStats('ChinookArtistsSeeder', [
            'processed' => count($this->chinookArtists),
            'skipped' => 0,
            'errors' => 0,
        ], $duration);
    }
    ...
    public function processBatch(array $batch): array
    {
        ...
        foreach ($batch as $artistData) {
            try {
                // Check if artist already exists
                $existingArtist = Artist::where('name', $artistData['name'])->first();
                ...
                $errors++;
                $this->command->error("Failed to create artist '{$artistData['name']}': " . $e->getMessage());
            }
        }
